...
import {
  COLABORACION_ACCEPTANCE_TEXT,
  COLABORACION_CLAUSES_EXCERPT,
  COLABORACION_FEE_SUMMARY,
  COLABORACION_LEGAL_LINKS,
  COLABORACION_META,
} from "@/lib/mandato/clauses-colaboracion";
...
export default function MandatoColaboracionSignWizard({
  prefill,
  alreadySigned,
  signedAt,
  signedEmail,
  panelHref = "/dashboard/professional",
}: Props) {
  ...
  const [professionalLegalName, setProfessionalLegalName] = useState(prefill.name ?? "");
  ...
  const validateStep1 = () => {
    if (!professionalLegalName.trim() || !professionalNif.trim() || !professionalAddress.trim()) {
      setError("Completa nombre/razón social, NIF y domicilio del profesional.");
      return false;
    }
  ...
  const validateStep2 = () => {
    ...
    if (!accepted) {
      setError("Debes aceptar las Condiciones Particulares y las Condiciones Generales.");
      return false;
    }
    return true;
  };
  ...
  const sign = async () => {
    ...
    setLoading(true);
    try {
      const res = await authFetch("/api/mandato/colaboracion/sign", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          professionalLegalName: professionalLegalName.trim(),
          professionalNif: professionalNif.trim(),
      ...
      if (!res.ok) {
        ...
        setError(data.error || "No se pudo firmar el acuerdo.");
        setLoading(false);
        return;
      }
      ...
      downloadBlob(blob, `acuerdo-colaboracion-diligenz-${date}.zip`);
      setDone(true);
      setLoading(false);
    } catch {
    ...
    return (
      <div className="mx-auto max-w-2xl rounded-2xl border border-[var(--brand-primary)]/15 bg-white p-8 text-center shadow-md">
        <h1 className="text-xl font-bold text-[var(--brand-primary)] sm:text-2xl">
          Acuerdo firmado correctamente
        </h1>
        <p className="mt-3 text-sm text-[var(--foreground)] opacity-90">
          Tu acuerdo de colaboración ha quedado registrado
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button
            type="button"
            onClick={() =>
              authFetch("/api/mandato/colaboracion/download")
                .then((r) => r.blob())
                .then((b) => downloadBlob(b, "acuerdo-colaboracion-diligenz.zip"))
            }
      ...
      <div className="rounded-2xl border border-[var(--brand-primary)]/15 bg-white p-6 shadow-md sm:p-8">
            ...
            <section>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <Field
                  ...
                  value={professionalLegalName}
                  onChange={setProfessionalLegalName}
                  className="sm:col-span-2"
                />
                <Field label="NIF *" value={professionalNif} onChange={setProfessionalNif} />
                <Field
                ...
                <Field
                  ...
                  onChange={setProfessionalAddress}
                  className="sm:col-span-2"
                />
        ...
        {step === 1 && (
          <div className="space-y-6">
            <div className="rounded-xl border border-[var(--brand-primary)]/15 bg-[var(--brand-bg)]/50">
              ...
              >
                Cláusulas del acuerdo — Léelas antes de firmar
              ...
              {clausesOpen && (
                <div className="max-h-64 overflow-y-auto border-t border-[var(--brand-primary)]/10 px-4 py-3 text-xs leading-relaxed text-[var(--foreground)] opacity-90 whitespace-pre-wrap">
                  ...
                  <div className="mt-4 space-y-1">
                    ...
                    <a
                      href={COLABORACION_LEGAL_LINKS.particulares}
                      target="_blank"
                    ...
                    <a
                      href={COLABORACION_LEGAL_LINKS.generales}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="block text-[var(--brand-primary)] underline"
                    >
            ...
            <ul className="list-disc space-y-1.5 pl-4 text-xs text-[var(--foreground)] opacity-85">
              {COLABORACION_FEE_SUMMARY.map((f) => (
                <li key={f}>{f}</li>
              ))}
            ...
            <label className="flex cursor-pointer items-start gap-3 text-xs text-[var(--foreground)] opacity-90 sm:text-sm">
              ...
              />
              <span>{COLABORACION_ACCEPTANCE_TEXT}</span>
            </label>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4 text-sm text-[var(--foreground)] opacity-90">
            ...
            <dl className="grid gap-3 text-xs sm:grid-cols-2 sm:text-sm">
              <Item k="Profesional" v={professionalLegalName} />
              <Item k="NIF" v={professionalNif} />
              <Item k="Domicilio" v={professionalAddress} className="sm:col-span-2" />
              <Item k="Representante" v={representativeName} />
              <Item k="DNI/NIF rep." v={representativeDni} />
              <Item k="Email" v={contactEmail} />
            </dl>
            <p className="text-xs opacity-75">
              Al pulsar «Firmar acuerdo» se generarán los dos PDF firmados, se empaquetarán en un ZIP
        ...
        <div className="mt-8 flex flex-wrap justify-between gap-3">
          ...
          {step < 2 ? (
            ...
            >
              {loading ? "Firmando…" : "Firmar acuerdo y descargar ZIP"}
            </button>
          )}
        </div>
      </div>
